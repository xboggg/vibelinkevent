...
function PhoneCarousel() {
  ...
  const [liveIdx, setLiveIdx] = useState<number | null>(null); // which phone is in iframe mode
  ...
  const goLive = () => setLiveIdx(active);
  const closeLive = () => setLiveIdx(null);
  ...
  return (
    <div className="relative h-[520px] md:h-[600px] flex items-center justify-center">
      {items.map((item, i) => {
        ...
        // Real phone-viewport screenshot lives at /phone-shots/<slug>.jpg
        // Fallback to the marketing thumbnail if the screenshot is missing.
        const phoneShot = `/phone-shots/${item.slug}.jpg`;
        const fallback = (item as { thumbnail?: string }).thumbnail || item.image;
        return (
          <motion.div
            key={item.id}
            ...
            onClick={() => {
              if (isLive) return; // don't dismiss on inner click
              if (isFront) return; // ignore taps on the front phone (use the "Try live" button)
              setActive(i);
            }}
          >
            <PhoneFrame
      ...
      {/* Screenshot fallback: if the phone-shots URL fails, browser will show the alt.
          We rely on browser auto-fallback via onerror; but to keep the DOM clean
          we set imgRef and swap src on error. */}
      <ImageFallbackBridge />

      {/* Front-of-stack controls */}
      {liveIdx === null && (
        <motion.div
      ...
      {/* Close button when in iframe mode */}
      {liveIdx !== null && (
        <button
          onClick={closeLive}
...
// Registers an onerror handler on all phone-shot images. If a screenshot 404s
// (e.g. haven't been generated yet for a new portfolio item), swap to the
// portfolio thumbnail so the carousel never shows a broken image.
function ImageFallbackBridge() {
  useEffect(() => {
    const handler = (e: Event) => {
      const el = e.target as HTMLImageElement;
      if (el.tagName === "IMG" && el.src.includes("/phone-shots/")) {
        const slug = el.src.split("/phone-shots/")[1].replace(".jpg", "");
        const item = items.find((it) => it.slug === slug);
        const fallback = item && ((item as { thumbnail?: string }).thumbnail || item.image);
