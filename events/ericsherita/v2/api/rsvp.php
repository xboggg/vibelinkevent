...
if ($method === 'GET') {
    ...
    foreach ($arr as $r) {
        if (($r['attending'] ?? 'yes') === 'yes') {
            ...
            $side = strtolower((string)($r['side'] ?? ''));
            if ($side === 'bride')      $bride += (int)($r['guests'] ?? 1);
            else if ($side === 'groom') $groom += (int)($r['guests'] ?? 1);
        } else {
            $decline++;
        }
    }
...
if ($method === 'POST') {
    ...
    $dietary   = clean_text((string)($body['dietary']   ?? ''), 200);
    ...
    if (count($arr) >= MAX_RSVP) {
        json_out(['error' => 'The guest list is now closed.'], 409);
    }
    $arr[] = [
    ...
    // Return the new counts for the ticker
